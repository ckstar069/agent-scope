...
pub fn set_template_path(path: String, state: State<'_, AppState>) -> Result<(), String> {
    ...
    let fingerprint =
        TemplateFingerprint::build(&path_buf).map_err(|e| format!("构建模板指纹失败: {}", e))?;
    ...
    save_template_path(&data_dir, &path_buf).map_err(|e| format!("保存模板路径失败: {}", e))?;
