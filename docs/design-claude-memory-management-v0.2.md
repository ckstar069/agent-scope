...
8. 项目级 rules（git repo 内为 `<repo_root>/.claude/rules/**/*.md`，非 git 目录为 `cwd/.claude/rules/**/*.md`，无 paths 的 → 无条件加载）
9. Auto Memory（~/.claude/projects/<project>/memory/MEMORY.md，前 200 行或前 25KB，取先到者）
   - `<project>` 的匹配策略：
     ...
     - git worktree：P1 不解析 `.git` 文件以恢复主 repo identity，也不做 cwd-encoded 近似匹配；返回 `auto_memory_worktree_unsupported` warning。worktree 共享 Auto Memory 语义留后续实现 ⚠️ P1 limitation
...
```
8. 路径触发 rules（git repo 内为 <repo_root>/.claude/rules/，非 git 目录为 cwd/.claude/rules/）
```
...
/// 5. 读取 project 级 rules（git repo 内用 repo_root/.claude/rules/，非 git 用 cwd/.claude/rules/）
/// 6. 读取 Auto Memory（普通 git repo 用 repo root 编码匹配；非 git 用 cwd 编码；worktree 不近似匹配）
...
1. 启动链构建（A 区域）:
   ...
   e. 项目级无条件 rules:
      - git 仓库内：<repo_root>/.claude/rules/**/*.md（无 paths frontmatter 的，递归扫描子目录）
...
2. path-scoped rules 发现（B 区域，单独列出）:
   ...
   b. 项目级 path-scoped rules:
      - git 仓库内：<repo_root>/.claude/rules/**/*.md（有 paths 的，递归扫描子目录）
      - 非 git 目录：cwd/.claude/rules/**/*.md
...
| 输入类型 | 行为 |
|---------|------|
| Registered project cwd | 项目级规则从 `<repo_root>/.claude/rules/` 读取（递归子目录，git repo 内用 repo root）；Auto Memory 通过 **repo root 编码路径** 匹配（普通 git repo） |
| Arbitrary cwd（未注册） | 项目级规则从 `<repo_root>/.claude/rules/` 读取（递归子目录，git repo 内用 repo root；Claude Code 不要求项目注册）；Auto Memory 通过 **repo root 编码路径** 匹配（普通 git repo） |
| 非 git 目录 | Auto Memory 通过 **cwd 编码路径** 匹配；若未找到匹配则记录 info warning，不阻断加载链模拟 |
| git worktree | P1 不解析 `.git` 文件，不做 cwd-encoded fallback，返回 `auto_memory_worktree_unsupported` warning。worktree 共享语义留后续实现 ⚠️ limitation |
...
2. **git worktree**：官方文档说明 worktree 与主 repo 共享 Auto Memory，但 P1 不解析 `.git` 文件以恢复主 repo identity，也不做 cwd-encoded 近似匹配（避免误展示）。返回 `auto_memory_worktree_unsupported` warning。worktree 共享语义留后续实现 ⚠️ P1 limitation
...
| Project | `<repo_root>/.claude/settings.json`（git repo 内）或 `<cwd>/.claude/settings.json`（非 git） | 当前项目 | ✅ 支持读取 |
| Local | `<repo_root>/.claude/settings.local.json`（git repo 内）或 `<cwd>/.claude/settings.local.json`（非 git） | 当前项目（本机私有） | ✅ 支持读取 |
...
- `<repo_root>/.claude/rules/**/*.md`（项目级，递归子目录；git repo 内使用 repo root，非 git 目录使用 cwd）
...
### 7.1 Claude Code 域子导航扩展

当前结构（v0.2 导航合并后，Claude 记忆并入 Claude Code 大域）：
```
Claude Code
├── 监控
│   ├── Agent 监控      (现有)
...
| `src/App.tsx` | MonitoringPage 扩展子页面类型（增加"记忆资产"、"加载模拟器"），Claude 记忆并入 Claude Code 域 |
| `src/components/Sidebar.tsx` | Claude Code 域子导航增加记忆资产和加载模拟器入口（监控组/记忆组） |
