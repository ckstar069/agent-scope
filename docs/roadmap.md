...
- [ ] **优化 CI artifact 生命周期，补充 artifact 查询规范**
  - `.gitlab-ci.yml`：给 `build:linux` / `build:windows` job 添加 `artifacts.expire_in: 1 day`，避免旧构建产物无限堆积
  - `docs/desktop-ci-cd-lessons.md`：新增条目，说明"为什么 job artifacts 会堆积"以及"外部系统应查询 Releases API 而非 Jobs API"
  - `docs/ci-cd-setup.md`：在 Release 章节补充 artifact 保留策略说明
- [ ] **修复 Pipeline #248 E2E 副标题文本不同步**
