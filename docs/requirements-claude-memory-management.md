...
| **Project Identity** | 项目的逻辑身份。Claude Code 实际使用编码后的 cwd 路径作为 Auto Memory 目录名，不以 git remote URL 识别 |
...
Memory Scanner（扫描文件系统，生成 logical asset）
    ↓
Project Identity Matcher（编码 cwd 路径 → 统一项目身份）
    ↓
...
| **Project Identity Matcher** | 通过编码 cwd 路径（`encode_cwd_path`）匹配 Auto Memory 目录，不依赖 git remote URL |
...
| 路径 | `~/.claude/projects/<encode_cwd_path(cwd)>/memory/` |
...
9. Auto Memory（`~/.claude/projects/<encode_cwd_path(cwd)>/memory/MEMORY.md`，前 200 行或前 25KB，取先到者）
...
- Auto memory 同步
- Project identity 绑定（编码 cwd 路径，与 Claude Code 的 Auto Memory 目录命名一致）
...
Claude 记忆已提升为**顶部一级域**，与项目监控、通用监控、设置并列：

```
顶部大域导航
├── 项目监控        (现有)
├── 通用监控        (现有)
├── Claude 记忆     (新增，v0.1 只读扫描)
└── 设置            (现有，不再承载 Claude 记忆)
    ├── 项目设置    (现有)
    └── 通用设置    (现有)
```
...
理由（v0.1 实现后的实际考虑）：
1. 记忆管理是高频查看功能，与"设置"的"低频配置"语义不同
2. 顶部大域 + 侧边栏子导航的架构已支撑 4 个域，扩展性足够
3. 信息架构与功能重要性匹配：记忆文件是 Claude Code 的核心上下文输入
