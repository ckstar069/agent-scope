...
它首先治理 Claude Code 的真实上下文注入链，同时为后续 AI 上下文资产治理预留统一模型：看见资产、看懂注入、评估质量、审查候选，再进入编辑、同步和跨工具适配。
...
8. 哪些记忆没有同步到另一台主力机？
9. 哪些记忆里可能泄露了 secret？
10. 从会话、扫描结果或外部上下文资产中提取的候选知识，应该保留、合并、归档还是丢弃？
11. 同一项目在 Claude、Codex、AGENTS.md、Cursor 等上下文资产之间是否已经重复或漂移？
...
| **可审查** | 将候选知识、重复项、冲突项和拆分建议汇入可追溯的审查队列 |
...
| **Memory Candidate** | 从会话、记忆资产、导入扫描或用户输入中提取的候选知识；保留来源、建议写入位置和审查状态，默认不直接写入真实资产 |
| **Governance Item** | 需要用户处理的治理项，包括候选知识、重复项、冲突项、拆分建议、同步阻断项和安全风险 |
| **Review Queue** | 汇总 Governance Item 的审查工作台，支持接受、拒绝、合并、归档和跳过 |
| **Context Pressure** | 某个 cwd 启动时由加载链资产大小、注入范围和重复度共同形成的上下文压力提示 |
...
Project Identity Matcher（普通 git repo：编码 repo root 路径；非 git：编码 cwd 路径）
    ↓
Load Analysis（加载链 / import / context pressure）
    ↓
Health & Conflict Detector（过长 / 重复 / 内容冲突 / 路径冲突 / 语义冲突 / 安全冲突）
    ↓
Governance Queue（候选知识 / 重复项 / 冲突项 / 拆分建议 / 阻断项）
    ↓
Sync / Backup / Restore
    ↓
...
| **Context Pressure Analyzer** | 基于加载链资产数、行数、字节数、重复度和 path-scoped 覆盖范围解释注入成本 |
| **Conflict Detector** | 检测内容冲突、路径冲突、语义冲突、安全冲突 |
| **Governance Queue** | 汇总候选知识、重复项、冲突项和拆分建议，保留来源并等待用户处理 |
| **Cross-tool Asset Inventory** | 只读盘点项目中的 Claude、Codex、AGENTS.md、Cursor 等上下文资产，检测覆盖范围差异和漂移 |
...
| 重复检测 v1 | 基于段落、标题、frontmatter 和轻量相似度发现重复 rules、重复候选知识和重复 import 内容 | 高 |
| Context Pressure | 汇总启动链资产数、行数、字节数和最重资产，提示拆分或收敛注入范围 | 高 |
| 结构冲突 v1 | 检测循环 import、过深 import、失效 paths、路径硬编码、重复覆盖范围等无需 NLP 的冲突 | 中 |
| 语义冲突 v2 | 检测语义层面的矛盾约束（如 pnpm vs npm、always vs never） | 低（需 NLP） |

健康检测不只输出问题列表，还应输出健康维度和行动建议。v1 健康评分至少区分加载压力、安全性、清洁度、一致性和可解释性；评分定义见 `docs/design-ai-context-governance-v0.1.md`。
...
- 重复记忆检测从 FR-09 v1 结果继续扩展到跨资产、跨工具候选
- 大文件拆分建议（CLAUDE.md → rules / skills）
- 记忆健康评分（加载压力、安全风险、重复、冲突、过长、过期、无引用）
- 支持将重复项、冲突项、拆分建议生成 Governance Item，进入 Review Queue

#### FR-17：会话 Transcript 分析

- 只读分析 `~/.claude/projects/<project>/<session>.jsonl`
- 从历史会话中提取 Memory Candidate，而不是直接写入真实记忆资产
- Candidate 保留 `source_session_id`、turn/snippet、建议类型、建议写入位置和产生原因
- Candidate 支持接受、拒绝、合并、归档和延后处理
...
- 生成前先给出 conversion preview，避免把未知资产直接覆盖为单一真源

#### FR-21：跨工具上下文资产盘点

- 只读发现项目中的 Claude、Codex、AGENTS.md、Cursor、Copilot 和 OpenCode 上下文资产
- 展示来源、作用域、加载或触发条件、与 Claude 资产的覆盖关系
- 检测明显重复、只在单一工具存在的规则和跨工具漂移
- 盘点阶段不自动导入、不自动覆盖；需要写入时先进入 Review Queue
...
| AC-27 | Review Queue 能追溯候选知识来源并处理接受/拒绝/合并/归档 | 手动测试 |
| AC-28 | 跨工具上下文资产盘点能发现重复与漂移候选 | 手动测试 |
